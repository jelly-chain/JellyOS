...
// ── 3f. Patch @jellyos/agent dist/tui/App.js — fix bare require() in ESM ────
// The agent is published as "type": "module" (ESM) but uses bare require() calls
// for @sinclair/typebox on line 160-161. In ESM, require is not defined.
// Fix: inject createRequire at the top of the file.
if (fs.existsSync(agentAppPath)) {
  let appSrc2 = fs.readFileSync(agentAppPath, "utf-8");
  if (appSrc2.includes('import { createRequire } from "module"')) {
    inf("@jellyos/agent App.js: createRequire already injected");
  } else {
    appSrc2 = 'import { createRequire } from "module";\nconst require = createRequire(import.meta.url);\n' + appSrc2;
    fs.writeFileSync(agentAppPath, appSrc2, "utf-8");
    ok("@jellyos/agent App.js: injected createRequire (fixes require() in ESM)");
  }
}

// ── 3f. Patch StatusBar.js — add rotation badge ────────────────────────────
const statusBarPath = path.join(ROOT, "node_modules", "@jellyos", "agent", "dist", "tui", "StatusBar.js");
if (fs.existsSync(statusBarPath)) {
  let sbSrc = fs.readFileSync(statusBarPath, "utf-8");
  if (sbSrc.includes("rotationBadge")) {
    inf("@jellyos/agent StatusBar.js: rotationBadge already present");
  } else {
    sbSrc = sbSrc.replace(
    ...
    sbSrc = sbSrc.replace(
      'children: [_jsx(Text, { color: JELLY_COLORS.accent, bold: true, children: "\\uD83E\\uDEBC JellyOS" }), _jsx(Text, { color: JELLY_COLORS.muted, children: modelShort })]',
      'children: [_jsx(Text, { color: JELLY_COLORS.accent, bold: true, children: "\\uD83E\\uDEBC JellyOS" }), _jsx(Text, { color: JELLY_COLORS.muted, children: modelShort }), rotationBadge ? _jsx(Text, { color: "#6b7280", children: rotationBadge }) : null]'
    );
    fs.writeFileSync(statusBarPath, sbSrc, "utf-8");
    ok("@jellyos/agent StatusBar.js: added rotation badge (↻N)");
  }
} else {
  inf("@jellyos/agent StatusBar.js not found — skipping");
}
