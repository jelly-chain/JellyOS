...
export function loadSkills(agent: ExtensionAPI, skillsDir?: string): number {
  ...
  if (!fs.existsSync(root)) {
    // Do NOT console.log/warn here — this runs inside Ink's render cycle and
    // raw stdout writes land in the middle of the TUI frame, causing visual
    // corruption (orphaned border lines, stacked frames).
    return 0;
  }
  ...
  for (const file of files) {
    ...
    try {
      ...
      // Silently skip — registering a skill mid-REPL-render is non-fatal
    }
  }

  // Return count only — the caller decides how to display it.
  // Console logging here would write into Ink's frame buffer.
  return loaded;
}
