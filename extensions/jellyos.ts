...
export default function jellyos(agent: ExtensionAPI): void {
  ...
  agent.on("session_start", async (_e, ctx) => {
    try {
      ...
      // The framework intercepts console.error and routes it to ui.notify,
      // which triggers a React re-render. Calling it during the initial
      // render cycle (fireHook runs inside useEffect mount) produces
      // stacked border frames. Defer the UI message to after mount.
      if (ctx?.ui?.notify) {
        setTimeout(() => {
      ...
      try {
        ...
        const logDir = path.join(JELLY_HOME, "logs");
        mkdirSync(logDir, { recursive: true });
        appendFileSync(
          path.join(logDir, "boot-errors.log"),
          `[${new Date().toISOString()}] ${bootErr?.message ?? String(bootErr)}\n`,
          "utf-8",
        );
  ...
  // ── Model rotation state (persisted to context.json) ──────────────────────
  // rotationSlots: [slot1..slot5] where slot1 = primary, slot2-5 = rotation
  // Each slot: { id: string, tier: string } | null
  function getRotationSlots(): Array<{ id: string; tier: string } | null> {
    const { existsSync, readFileSync } = require("node:fs");
    try {
      const ctxPath = path.join(JELLY_HOME, "context.json");
      if (!existsSync(ctxPath)) return [null, null, null, null, null];
      const store = JSON.parse(readFileSync(ctxPath, "utf-8"));
      return store.rotationSlots ?? [null, null, null, null, null];
    } catch { return [null, null, null, null, null]; }
  }
  function saveRotationSlots(slots: Array<{ id: string; tier: string } | null>) {
    const { writeFileSync, readFileSync, existsSync, mkdirSync } = require("node:fs");
    mkdirSync(JELLY_HOME, { recursive: true });
    try {
      const ctxPath = path.join(JELLY_HOME, "context.json");
  ...
  function getSlotModelId(slot: number): string | null {
    const slots = getRotationSlots();
    return slots[slot]?.id ?? null;
  }
  function setSlotModelId(slot: number, id: string, tier: string) {
    const slots = getRotationSlots();
    slots[slot] = { id, tier };
  ...
  // Helper: find models by tier, sorted by cost, excluding already-used slots
  function findModelsByTier(tier: string, usedIds: string[], limit = 10) {
    const pool = modelRegistry.getPool(tier);
    return pool
      .filter(tm => tm.available && tm.failures < 3 && !usedIds.includes(tm.model.id))
      .sort((a, b) => a.costPer1K - b.costPer1K)
      .slice(0, limit);
  }

  // Helper: auto-detect compatible models for a tier
  function autoFillSlots(tier: string, count: number = 5): Array<{ id: string; tier: string } | null> {
    ...
    const result: Array<{ id: string; tier: string } | null> = [];

    // Round-robin across tiers to get diverse models
    let tierIdx = 0;
    while (result.length < count) {
      ...
      tierIdx++;
      // Safety: if we've gone through all tiers twice with no new models, stop
      if (tierIdx >= tiers.length * count) break;
    }
    return result;
  }

  // Helper: format a slot for display
  function formatSlot(slotIdx: number, slot: { id: string; tier: string } | null, isPrimary: boolean = false) {
    ...
    const cost = (() => {
      const tm = (modelRegistry as any).modelMap?.get(slot.id);
      if (!tm) return "?";
      return tm.costPer1K <= 0 ? "FREE" : `$${(tm.costPer1K / 1_000_000_000).toFixed(4)}/1K`;
    })();
  ...
  agent.registerCommand("model", {
    ...
    async handler(args, ctx) {
      ...
      // ── /model setN <id> — set a specific rotation slot (N = 1-5) ────────
      const setMatch = arg.match(/^set([1-5])\s+(.+)$/);
      if (setMatch) {
        const slotNum = parseInt(setMatch[1]) - 1; // 0-indexed
        const id = setMatch[2].trim();
        // Determine tier from registry
        const tm = (modelRegistry as any).modelMap?.get(id);
        const tier = tm?.tier ?? "worker";
        setSlotModelId(slotNum, id, tier);
        // If setting slot 1 (primary), also update DEFAULT_MODEL
        if (slotNum === 0) {
      ...
      // ── /model clearN — clear a rotation slot ────────────────────────────
      ...
      if (arg === "free" || arg === "pro" || arg === "turbo") {
        ...
        const lines = [
          ctx.ui.theme.fg("accent", `🪼 Model Rotation — ${arg.toUpperCase()} tier auto-filled`),
          "",
        ];
        for (let i = 0; i < 5; i++) {
      ...
      // ── /model set <id> — direct set primary (backward compat) ───────────
      ...
      // ── /model | /model <query> — interactive selector or text list ───────
  ...
  agent.registerCommand("rotation", {
    ...
    async handler(args, ctx) {
      ...
      // ── /rotation auto <tier> — auto-fill all 5 slots ────────────────────
      if (arg.startsWith("auto ")) {
        ...
        const lines = [
          ctx.ui.theme.fg("accent", `🪼 Rotation auto-filled — ${tier.toUpperCase()} tier`),
          "",
        ];
        for (let i = 0; i < 5; i++) {
          lines.push(formatSlot(i, newSlots[i], i === 0));
      ...
      // ── /rotation — show current rotation config ─────────────────────────
      const slots = getRotationSlots();
      const envFile = path.join(JELLY_HOME, ".env");
      const current = (() => {
        if (!existsSync(envFile)) return process.env.DEFAULT_MODEL ?? "not set";
        const m = readFileSync(envFile, "utf-8").match(/^DEFAULT_MODEL=(.+)$/m);
        return m?.[1]?.trim() ?? process.env.DEFAULT_MODEL ?? "not set";
      })();

      const lines = [
        ctx.ui.theme.fg("accent", "🪼 Model Rotation"),
        "",
