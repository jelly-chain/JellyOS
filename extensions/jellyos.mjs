...
function keccak256Hex(data) {
  ...
  return ethers.keccak256(data).slice(2);
}
    ...
    try {
      if (wallet.keyType === "secp256k1") {
        try {
          ...
          return signingKey.sign("0x" + hash3).serialized;
        } catch {
        }
        const ecdh = crypto.createECDH("secp256k1");
...
var AutoVault = class {
  ...
  readThreshold() {
    try {
      const { existsSync: existsSync5, readFileSync: readFileSync4 } = __require("fs");
      const ctxPath = __require("path").resolve(__require("os").homedir(), ".jelly", "context.json");
      if (existsSync5(ctxPath)) {
        const ctx = JSON.parse(readFileSync4(ctxPath, "utf-8"));
...
var SignalEngine = class extends EventEmitter {
  feeds;
  logger;
  signals = [];
  maxSignals = 50;
...
function findSkillFiles(dir, root) {
  const results = [];
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        results.push(path2.relative(root, fullPath));
      }
    }
  } catch {
  }
...
function loadSkills(agent, skillsDir) {
  const root = skillsDir ?? path2.resolve(
    // When running from extensions/jellyos.ts (compiled or ESM)
    // __dirname works in CJS; for ESM we fall back to cwd-relative
    typeof __dirname !== "undefined" ? path2.join(__dirname, "..", "skills") : path2.join(process.cwd(), "skills")
  );
  ...
  for (const file of files) {
    const filePath = path2.join(root, file);
    let content;
    try {
...
var JELLY_HOME = process.env.JELLYOS_HOME ?? path3.join(os.homedir(), ".jelly");
var CHAIN_NETWORK = {
  bsc: "bnb-mainnet",
  ...
  ws.on("message", (raw) => {
    try {
      ...
      if (msg.type === "agent_message" && msg.text) {
        ...
        const { writeFileSync: writeFileSync3, readFileSync: readFileSync4, existsSync: existsSync5, mkdirSync: mkdirSync4 } = __require("node:fs");
        const ctxPath = path3.join(JELLY_HOME, "context.json");
        mkdirSync4(JELLY_HOME, { recursive: true });
        const store = existsSync5(ctxPath) ? JSON.parse(readFileSync4(ctxPath, "utf-8")) : {};
...
function _checkAlerts(feeds) {
  if (!feeds) return;
  const { existsSync: existsSync5, readFileSync: readFileSync4, writeFileSync: writeFileSync3 } = __require("node:fs");
  const alertsPath = path3.join(JELLY_HOME, "alerts.json");
  if (!existsSync5(alertsPath)) return;
  let alerts = [];
  try {
    alerts = JSON.parse(readFileSync4(alertsPath, "utf-8"));
  } catch {
    return;
  }
...
async function _pollWatchedWallets() {
  ...
  const watchPath = path3.join(JELLY_HOME, "watched-wallets.json");
  if (!existsSync5(watchPath)) return;
  let wallets = [];
  try {
    wallets = JSON.parse(readFileSync4(watchPath, "utf-8"));
  } catch {
    return;
  }
...
function _logTrade(entry) {
  try {
    ...
    const dir = path3.join(JELLY_HOME, "journal");
    mkdirSync4(dir, { recursive: true });
    appendFileSync2(path3.join(dir, `${date}.jsonl`), JSON.stringify(entry) + "\n", "utf-8");
  } catch {
  }
}
  ...
  agent.on("session_start", async (_e, ctx) => {
    try {
      ...
      try {
        ...
        positionManager = new PositionManager2(new Metrics2(new Logger2("Trading")));
      } catch {
      }
      let getPnL = () => {
        if (positionManager) {
          try {
            return positionManager.getTotalPnL?.() ?? 0;
          } catch {
            return 0;
          }
        }
      ...
      signals.on("signal_generated", (sig) => {
      ...
      try {
        ...
        if (skillCount > 0) {
          setTimeout(() => ctx.ui.setStatus("skills", `${skillCount} skills`), 2e3);
        }
      } catch {
      }
      try {
      ...
      _statusS = signals;
      _statusW = wallet;
      _statusAny._positionManager = positionManager;
      ...
      _statusAny.serviceHealth = serviceHealth;
      ...
      if (ctx?.ui?.notify) {
        setTimeout(() => {
          ctx.ui.notify("\u26A0 JellyOS boot error \u2014 check ~/.jelly/.env config");
        }, 100);
      }
      try {
        const { appendFileSync: appendFileSync2, mkdirSync: mkdirSync4 } = __require("node:fs");
        const logDir = path3.join(JELLY_HOME, "logs");
        mkdirSync4(logDir, { recursive: true });
        appendFileSync2(
          path3.join(logDir, "boot-errors.log"),
          `[${(/* @__PURE__ */ new Date()).toISOString()}] ${bootErr?.message ?? String(bootErr)}
`,
          "utf-8"
        );
      } catch {
  ...
  agent.on("before_agent_start", async (_e, _ctx) => {
    ...
    try {
      ...
      const promptPath = path3.join(_esm_dirname, "..", "prompts", "jellyos.md");
      basePrompt = readFileSync4(promptPath, "utf-8");
    } catch {
    }
    ...
    const effectLine = (() => {
      try {
        ...
        const ctxPath = path3.join(JELLY_HOME, "context.json");
        return existsSync5(ctxPath) ? `effect_level: ${JSON.parse(readFileSync4(ctxPath, "utf-8")).effect_level ?? "normal"}` : "effect_level: normal";
      } catch {
...
${_dashboardMessages.splice(0, _dashboardMessages.length).map((m) => `- ${m.text}`).join("\n")}` : "";
    ...
    try {
      ...
      const ctxPath = path3.join(JELLY_HOME, "context.json");
      if (existsSync5(ctxPath)) {
        const store = JSON.parse(readFileSync4(ctxPath, "utf-8"));
        const tasks = (store.schedule || []).filter((t) => t.active);
        if (tasks.length > 0) {
          schedBlock = `
...
${tasks.map((t) => `- ${t.task}`).join("\n")}`;
  ...
  agent.registerCommand("panic", {
    ...
    async handler(_args, ctx) {
      const { existsSync: existsSync5, readFileSync: readFileSync4, writeFileSync: writeFileSync3, mkdirSync: mkdirSync4 } = __require("node:fs");
      const lines = [
      ...
      if (existsSync5(ctxPath)) {
        try {
          store = JSON.parse(readFileSync4(ctxPath, "utf-8"));
...
Positions emergency-closed (${openPositions.length}):`);
  ...
  agent.registerCommand("effect", {
    ...
    async handler(args, ctx) {
      ...
      if (!level) {
        const { readFileSync: readFileSync5, existsSync: existsSync6 } = __require("node:fs");
        const ctxPath2 = __require("node:path").join(JELLY_HOME, "context.json");
        const current = existsSync6(ctxPath2) ? JSON.parse(readFileSync5(ctxPath2, "utf-8")).effect_level ?? "normal" : "normal";
...
Choose: eco | normal | turbo | max`);
      ...
      const { readFileSync: readFileSync4, writeFileSync: writeFileSync3, existsSync: existsSync5, mkdirSync: mkdirSync4 } = __require("node:fs");
...
${desc[level]}`));
  ...
  agent.registerCommand("wallets", {
    ...
    async handler(_args, ctx) {
      ...
      const { existsSync: existsSync5, readFileSync: readFileSync4 } = __require("node:fs");
      const addrFile = path3.join(JELLY_HOME, "vault-addresses.json");
      if (existsSync5(addrFile)) {
        lines.push("\nVault addresses (cold -- only accessible with your saved private key):");
        const a = JSON.parse(readFileSync4(addrFile, "utf-8"));
...
Use the agent: ask "scan whale ${addr}" for full on-chain analysis`);
  ...
  agent.registerCommand("chain", {
    ...
    async handler(args, ctx) {
      ...
      if (!chain) {
        const store2 = existsSync5(ctxPath) ? JSON.parse(readFileSync4(ctxPath, "utf-8")) : {};
...
${tasks.map((t) => `  \u2022 ${JSON.stringify(t)}`).join("\n")}` : "No scheduled tasks"
  ...
  function getRotationSlots() {
    ...
    try {
      const ctxPath = path3.join(JELLY_HOME, "context.json");
      if (!existsSync5(ctxPath)) return [null, null, null, null, null];
      const store = JSON.parse(readFileSync4(ctxPath, "utf-8"));
      return store.rotationSlots ?? [null, null, null, null, null];
    } catch {
      return [null, null, null, null, null];
    }
  }
  function saveRotationSlots(slots) {
    const { writeFileSync: writeFileSync3, readFileSync: readFileSync4, existsSync: existsSync5, mkdirSync: mkdirSync4 } = __require("node:fs");
    mkdirSync4(JELLY_HOME, { recursive: true });
    try {
      const ctxPath = path3.join(JELLY_HOME, "context.json");
  ...
  function getSlotModelId(slot) {
    const slots = getRotationSlots();
    return slots[slot]?.id ?? null;
  }
  function setSlotModelId(slot, id, tier) {
    const slots = getRotationSlots();
    slots[slot] = { id, tier };
    saveRotationSlots(slots);
  }
  ...
  function findModelsByTier(tier, usedIds, limit = 10) {
    const pool = modelRegistry.getPool(tier);
    return pool.filter((tm) => tm.available && tm.failures < 3 && !usedIds.includes(tm.model.id)).sort((a, b) => a.costPer1K - b.costPer1K).slice(0, limit);
  }
  function autoFillSlots(tier, count = 5) {
    const tierMap = {
      free: ["free"],
      pro: ["orchestrator", "analyst", "worker"],
      turbo: ["orchestrator", "analyst", "worker", "free"]
    };
    const tiers = tierMap[tier] ?? ["worker"];
    const usedIds = [];
    const result = [];
    let tierIdx = 0;
    while (result.length < count) {
      const currentTier = tiers[tierIdx % tiers.length];
      const models = findModelsByTier(currentTier, usedIds, 1);
      if (models.length > 0) {
        const m = models[0];
        result.push({ id: m.model.id, tier: currentTier });
        usedIds.push(m.model.id);
      } else {
        result.push(null);
      }
      tierIdx++;
      if (tierIdx >= tiers.length * count) break;
    }
    return result;
  }
  function formatSlot(slotIdx, slot, isPrimary = false) {
    const label = isPrimary ? "1 (primary)" : `${slotIdx + 1}`;
    if (!slot) {
      return `  ${label.padEnd(12)} [empty]`;
    }
    const shortId = slot.id.split("/").pop() ?? slot.id;
    const cost = (() => {
      const tm = modelRegistry.modelMap?.get(slot.id);
      if (!tm) return "?";
      return tm.costPer1K <= 0 ? "FREE" : `$${(tm.costPer1K / 1e9).toFixed(4)}/1K`;
    })();
    return `  ${label.padEnd(12)} [${slot.tier.padEnd(12)}] ${shortId.padEnd(30)} ${cost}`;
  }
  agent.registerCommand("model", {
    description: "Model picker & rotation -- /model | /model free|pro|turbo | /model setN <id> | /model clearN",
    async handler(args, ctx) {
      const { writeFileSync: writeFileSync3, readFileSync: readFileSync4, existsSync: existsSync5, mkdirSync: mkdirSync4 } = __require("node:fs");
      const envFile = path3.join(JELLY_HOME, ".env");
      mkdirSync4(JELLY_HOME, { recursive: true });
      const arg = args.trim();
      const setMatch = arg.match(/^set([1-5])\s+(.+)$/);
      if (setMatch) {
        const slotNum = parseInt(setMatch[1]) - 1;
        const id = setMatch[2].trim();
        const tm = modelRegistry.modelMap?.get(id);
        const tier = tm?.tier ?? "worker";
        setSlotModelId(slotNum, id, tier);
        if (slotNum === 0) {
          const content = existsSync5(envFile) ? readFileSync4(envFile, "utf-8") : "";
        ...
        ctx.ui.notify(ctx.ui.theme.fg("accent", `\u2713 Slot ${slotNum + 1} set to: ${shortId} [${tier}]`));
        return;
      }
      const clearMatch = arg.match(/^clear([1-5])$/);
      if (clearMatch) {
        const slotNum = parseInt(clearMatch[1]) - 1;
        clearSlot(slotNum);
        ctx.ui.notify(ctx.ui.theme.fg("accent", `\u2713 Slot ${slotNum + 1} cleared`));
        return;
      }
      if (arg === "free" || arg === "pro" || arg === "turbo") {
        const newSlots = autoFillSlots(arg, 5);
        saveRotationSlots(newSlots);
        if (newSlots[0]) {
          const content = existsSync5(envFile) ? readFileSync4(envFile, "utf-8") : "";
          const re = /^DEFAULT_MODEL=.*$/m;
          const line = `DEFAULT_MODEL=${newSlots[0].id}`;
          writeFileSync3(envFile, re.test(content) ? content.replace(re, line) : content + "\n" + line + "\n", "utf-8");
          process.env.DEFAULT_MODEL = newSlots[0].id;
        }
        const lines = [
          ctx.ui.theme.fg("accent", `\u{1FABC} Model Rotation \u2014 ${arg.toUpperCase()} tier auto-filled`),
          ""
        ];
        for (let i = 0; i < 5; i++) {
          lines.push(formatSlot(i, newSlots[i], i === 0));
        }
        lines.push(
          "",
          ctx.ui.theme.fg("muted", `Primary: ${newSlots[0]?.id ?? "not set"}`),
          ctx.ui.theme.fg("muted", "Use /model setN <id> to customize any slot")
        );
        ctx.ui.notify(lines.join("\n"));
        return;
      }
      if (arg.startsWith("set ")) {
        ...
        const tm = modelRegistry.modelMap?.get(id);
        const tier = tm?.tier ?? "worker";
        setSlotModelId(0, id, tier);
        const content = existsSync5(envFile) ? readFileSync4(envFile, "utf-8") : "";
        const re = /^DEFAULT_MODEL=.*$/m;
        const line = `DEFAULT_MODEL=${id}`;
        writeFileSync3(envFile, re.test(content) ? content.replace(re, line) : content + "\n" + line + "\n", "utf-8");
        process.env.DEFAULT_MODEL = id;
        ctx.ui.notify(ctx.ui.theme.fg("accent", `\u2713 Primary model set to: ${id}
Restart jellyos to apply.`));
        return;
      }
      if (typeof ctx.ui.showModelSelector === "function") {
        ctx.ui.showModelSelector(arg || "");
      } else {
        const current = (() => {
          if (!existsSync5(envFile)) return process.env.DEFAULT_MODEL ?? "anthropic/claude-sonnet-4-5";
          const m = readFileSync4(envFile, "utf-8").match(/^DEFAULT_MODEL=(.+)$/m);
        ...
        lines.push(
          "",
          ctx.ui.theme.fg("accent", "Rotation slots:")
        );
        for (let i = 0; i < 5; i++) {
          lines.push(formatSlot(i, slots[i], i === 0));
        }
        lines.push(
          "",
          ctx.ui.theme.fg("muted", `Current: ${current}`),
          ctx.ui.theme.fg("muted", "Usage: /model free|pro|turbo | /model setN <id> | /model clearN")
        );
        ctx.ui.notify(lines.join("\n"));
      }
    }
  });
  agent.registerCommand("rotation", {
    description: "Show and manage model rotation slots -- /rotation | /rotation auto <free|pro|turbo>",
    async handler(args, ctx) {
      const { writeFileSync: writeFileSync3, readFileSync: readFileSync4, existsSync: existsSync5, mkdirSync: mkdirSync4 } = __require("node:fs");
      const arg = args.trim();
      if (arg.startsWith("auto ")) {
        const tier = arg.slice(5).trim().toLowerCase();
        if (tier !== "free" && tier !== "pro" && tier !== "turbo") {
          ctx.ui.notify(ctx.ui.theme.fg("error", "Usage: /rotation auto <free|pro|turbo>"));
          return;
        }
        const newSlots = autoFillSlots(tier, 5);
        saveRotationSlots(newSlots);
        if (newSlots[0]) {
          const envFile2 = path3.join(JELLY_HOME, ".env");
          mkdirSync4(JELLY_HOME, { recursive: true });
          const content = existsSync5(envFile2) ? readFileSync4(envFile2, "utf-8") : "";
          const re = /^DEFAULT_MODEL=.*$/m;
          const line = `DEFAULT_MODEL=${newSlots[0].id}`;
          writeFileSync3(envFile2, re.test(content) ? content.replace(re, line) : content + "\n" + line + "\n", "utf-8");
          process.env.DEFAULT_MODEL = newSlots[0].id;
        }
        const lines2 = [
          ctx.ui.theme.fg("accent", `\u{1FABC} Rotation auto-filled \u2014 ${tier.toUpperCase()} tier`),
          ""
        ];
        for (let i = 0; i < 5; i++) {
          lines2.push(formatSlot(i, newSlots[i], i === 0));
        }
        lines2.push(
          "",
          ctx.ui.theme.fg("muted", "Use \u2191\u2193 to browse, Enter to select, /model setN <id> to customize")
        );
      ...
      const envFile = path3.join(JELLY_HOME, ".env");
      const current = (() => {
        if (!existsSync5(envFile)) return process.env.DEFAULT_MODEL ?? "not set";
        const m = readFileSync4(envFile, "utf-8").match(/^DEFAULT_MODEL=(.+)$/m);
        return m?.[1]?.trim() ?? process.env.DEFAULT_MODEL ?? "not set";
      })();
      const lines = [
        ctx.ui.theme.fg("accent", "\u{1FABC} Model Rotation"),
        "",
        ctx.ui.theme.fg("muted", `Active model: ${current}`),
        ""
      ];
      for (let i = 0; i < 5; i++) {
        lines.push(formatSlot(i, slots[i], i === 0));
      ...
      lines.push(
        "",
        ctx.ui.theme.fg("muted", "Commands:"),
        "  /model free          Auto-fill all 5 slots with free models",
        "  /model pro           Auto-fill with paid models (orchestrator/analyst/worker)",
        "  /model turbo          Auto-fill with all tiers including free",
        "  /model set3 <id>     Set slot 3 to a specific model ID",
        "  /model clear2        Clear slot 2",
        "  /rotation auto free  Same as /model free"
      );
      ctx.ui.notify(lines.join("\n"));
    }
  });
  agent.registerCommand("config", {
    ...
    async handler(_args, ctx) {
      ...
      if (existsSync5(envFile)) {
        for (const line of readFileSync4(envFile, "utf-8").split("\n")) {
  ...
  agent.registerCommand("skills", {
    ...
    async handler(_args, ctx) {
      const { existsSync: existsSync5, readdirSync: readdirSync2 } = __require("node:fs");
      const skillsDir = path3.join(JELLY_HOME, "skills");
      if (!existsSync5(skillsDir)) {
        ctx.ui.notify("No Jelly Skills installed.\nInstall during setup or clone to ~/.jelly/skills/");
        return;
      }
      const dirs = readdirSync2(skillsDir, { withFileTypes: true }).filter((d) => d.isDirectory());
      if (dirs.length === 0) {
        ctx.ui.notify("No skills installed yet");
        return;
      }
      const lines = dirs.map((d) => {
        const hasCmd = existsSync5(path3.join(skillsDir, d.name, "jelly-command.json"));
...
${lines.join("\n")}`);
  ...
  agent.registerCommand("ping", {
    ...
    async handler(_args, ctx) {
      ...
      checks.push(`  JELLY_HOME       ${path3.join(JELLY_HOME, ".env") ? "\u2713" : "\u2717"} ${JELLY_HOME}`);
  ...
  agent.registerCommand("agents", {
    ...
    async handler(args, ctx) {
      ...
      if (!cmd || cmd === "status") {
        const ctxPath = path3.join(JELLY_HOME, "context.json");
        const effect = existsSync5(ctxPath) ? JSON.parse(readFileSync4(ctxPath, "utf-8")).effect_level ?? "normal" : "normal";
      ...
      if (cmd.startsWith("analyze ") || cmd.startsWith("analyze")) {
        ...
        const ctxPath2 = path3.join(JELLY_HOME, "context.json");
  ...
  (() => {
    const { existsSync: existsSync5, readdirSync: readdirSync2, readFileSync: readFileSync4 } = __require("node:fs");
    const skillsDir = path3.join(JELLY_HOME, "skills");
    ...
    try {
      ...
      for (const skill of skills) {
        const cmdFile = path3.join(skillsDir, skill, "jelly-command.json");
        if (!existsSync5(cmdFile)) continue;
        try {
          const cmd = JSON.parse(readFileSync4(cmdFile, "utf-8"));
...
${items}`
  ...
  agent.registerTool({
    ...
    async execute(_id, params) {
      const { readFileSync: readFileSync4, existsSync: existsSync5 } = await import("node:fs");
      const ctxPath = path3.join(JELLY_HOME, "context.json");
      if (!existsSync5(ctxPath)) return text(`No context stored yet`);
      const store = JSON.parse(readFileSync4(ctxPath, "utf-8"));
  ...
  agent.registerTool({
    ...
    async execute(_id, params) {
      ...
      const { readFileSync: readFileSync4, writeFileSync: writeFileSync3, existsSync: existsSync5, mkdirSync: mkdirSync4 } = await import("node:fs");
      mkdirSync4(JELLY_HOME, { recursive: true });
      const ctxPath = path3.join(JELLY_HOME, "context.json");
      const store = existsSync5(ctxPath) ? JSON.parse(readFileSync4(ctxPath, "utf-8")) : {};
...
${msg.trim() || err.message}`);
  ...
  agent.registerTool({
    ...
    async execute(_id, params) {
      const { readFileSync: readFileSync4, statSync: statSync2, existsSync: existsSync5 } = __require("node:fs");
      ...
      if (!existsSync5(resolvedPath)) return text(`File not found: ${resolvedPath}`);
      ...
      const raw = readFileSync4(resolvedPath);
...
[truncated -- ${raw.length} bytes total, showing first ${maxBytes}]` : "";
  ...
  agent.registerTool({
    ...
    async execute(_id, params) {
      const { writeFileSync: writeFileSync3, appendFileSync: appendFileSync2, existsSync: existsSync5, mkdirSync: mkdirSync4 } = __require("node:fs");
      const resolvedPath = params.path.replace(/^~/, os.homedir());
      ...
      if (mode === "overwrite" && existsSync5(resolvedPath) && !params.confirm) {
        return text(`\u26A0\uFE0F ${resolvedPath} already exists. Call again with confirm: true to overwrite.`);
      }
      mkdirSync4(path3.dirname(resolvedPath), { recursive: true });
      if (mode === "append") {
  ...
  agent.registerCommand("snapshot", {
    ...
    async handler(_args, ctx) {
      ...
      const md = lines.join("\n");
      const dir = path3.join(JELLY_HOME, "snapshots");
      mkdirSync4(dir, { recursive: true });
      const file = path3.join(dir, `${ts}.md`);
...
${md.slice(0, 800)}${md.length > 800 ? "\n..." : ""}`);
  ...
  agent.registerCommand("journal", {
    ...
    async handler(args, ctx) {
      ...
      const files = readdirSync2(dir).filter((f) => f.endsWith(".jsonl")).sort().reverse();
      if (!files.length) {
        ctx.ui.notify("No journal entries yet");
        return;
      }
...
`];
      ...
      outer: for (const file of files) {
        const rows = readFileSync4(path3.join(dir, file), "utf-8").split("\n").filter(Boolean).reverse();
...
Override port with JELLY_WEBHOOK_PORT in ~/.jelly/.env`
  ...
  agent.registerTool({
    ...
    async execute(_id, params) {
      const { existsSync: existsSync5, readFileSync: readFileSync4, writeFileSync: writeFileSync3 } = __require("node:fs");
      const alertsPath = path3.join(JELLY_HOME, "alerts.json");
      if (!existsSync5(alertsPath)) return text("No alerts configured");
      let alerts = [];
      try {
        alerts = JSON.parse(readFileSync4(alertsPath, "utf-8"));
      } catch {
  ...
  agent.registerTool({
    ...
    async execute(_id, params) {
      const { existsSync: existsSync5, readFileSync: readFileSync4, readdirSync: readdirSync2 } = __require("node:fs");
      const dir = path3.join(JELLY_HOME, "journal");
      if (!existsSync5(dir)) return text("No journal entries yet");
      const limit = params.limit ?? 20;
      const targetFile = params.date ? path3.join(dir, `${params.date}.jsonl`) : (() => {
        const files = readdirSync2(dir).filter((f) => f.endsWith(".jsonl")).sort().reverse();
        return files[0] ? path3.join(dir, files[0]) : null;
      })();
      if (!targetFile || !existsSync5(targetFile)) return text("No journal entries found for that date");
      const rows = readFileSync4(targetFile, "utf-8").split("\n").filter(Boolean);
...
Note: Demo backtest with synthetic data. Use real OHLCV for production.`
  ...
  agent.registerTool({
    ...
    async execute(_id, p) {
      ...
      const store = existsSync5(ctxPath) ? JSON.parse(readFileSync4(ctxPath, "utf-8")) : {};
      const tasks = store.schedule || [];
      const idx = tasks.findIndex((t) => t.task === p.task);
      if (p.active === false && idx >= 0) {
        tasks.splice(idx, 1);
      } else if (idx < 0) {
        tasks.push({ task: p.task, created: Date.now(), active: true });
      }
...
${tasks.map((t) => `  \u2022 ${t.task}`).join("\n")}`);
  ...
  agent.registerTool({
    ...
    async execute(_id, p) {
      const lines = ["\u{1FABC} JellyOS Daily Briefing", `\u{1F4C5} ${(/* @__PURE__ */ new Date()).toLocaleDateString()}`, ""];
      const ticks = priceFeed.getAll();
      if (ticks.length > 0) {
        lines.push("\u{1F4CA} Prices:");
        for (const t of ticks.slice(0, 8)) {
          const emoji = t.change24h >= 0 ? "\u{1F7E2}" : "\u{1F534}";
          lines.push(`  ${emoji} ${t.symbol} $${t.price.toLocaleString()} (${t.change24h >= 0 ? "+" : ""}${t.change24h.toFixed(2)}%)`);
        }
      }
      const report = newsFeed.getLatest();
      if (report) {
        const mood = report.avgSentiment > 0.2 ? "\u{1F7E2} Bullish" : report.avgSentiment < -0.2 ? "\u{1F534} Bearish" : "\u{1F7E1} Neutral";
        lines.push(`
\u{1F4F0} Sentiment: ${mood} (${(report.avgSentiment * 100).toFixed(0)}%)`);
...
\u26A1 Signals (${sigs.length}):`);
      ...
      if (vault && !vault.isLocked()) {
        lines.push(`
\u{1F510} Vault: $${vault.getStats().balance?.toFixed(2)} (${vault.getStats().entries} entries)`);
      }
      lines.push(`
\u{1F4A1} Trade Idea:`);
      ...
      if (p.send_to === "telegram" || p.send_to === "both") {
        _tgSend(briefing).catch(() => {
        });
      }
      if (p.send_to === "discord" || p.send_to === "both") {
        _dcSend(briefing).catch(() => {
        });
      }
...
\u2713 Sent to ${p.send_to}` : ""));
  ...
  agent.registerCommand("debate", {
    ...
    async handler(args, ctx) {
      if (!args.trim()) {
        ctx.ui.notify("Usage: /debate <topic> -- e.g. /debate Should I buy ETH today?");
        return;
      }
      const topic = args.trim();
      ctx.ui.notify([
        `\u{1FABC} Debate: ${topic}`,
        "",
  ...
  agent.registerCommand("whale", {
    ...
    async handler(_args, ctx) {
      const watchPath = path3.join(JELLY_HOME, "watched-wallets.json");
      const { existsSync: existsSync5, readFileSync: readFileSync4 } = __require("node:fs");
      const wallets = existsSync5(watchPath) ? JSON.parse(readFileSync4(watchPath, "utf-8")) : [];
      ctx.ui.notify([
        `\u{1F40B} Whale Tracker`,
...
\u26A0 Polling mode. For real-time, use Alchemy Mempool WebSocket.`
  ...
  agent.registerCommand("arb", {
    ...
    async handler(args, ctx) {
      if (!args.trim()) {
        ctx.ui.notify("Usage: /arb <symbol> -- e.g. /arb ETH");
        return;
      }
...
Ask the agent: "scan arbitrage for ${args.trim()}"`);
  ...
  agent.registerCommand("health", {
    ...
    async handler(_args, ctx) {
      const h = _statusAny?.serviceHealth;
      if (!h) {
        ctx.ui.notify("Health data not available \u2014 services still booting");
        return;
      }
      const lines = ["SERVICE HEALTH", "\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500"];
      for (const [svc, status] of h) {
        const icon = status === "ok" ? "\u{1F7E2}" : status === "degraded" ? "\u{1F7E1}" : "\u{1F534}";
        lines.push(`  ${icon} ${svc.padEnd(16)} ${status.toUpperCase()}`);
      }
      const mem = `${(process.memoryUsage().rss / 1e6).toFixed(0)}MB`;
      lines.push(`
  \u{1F7E2} node         ${process.version} (${mem})`);
      ctx.ui.notify(lines.join("\n"));
    }
  });
}
